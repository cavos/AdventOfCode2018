...
pub fn solve_part1(){
    ...
    let result = buffered.lines().map(|l| l.unwrap()).fold(0i32, |freq, x| freq + x.parse::<i32>().unwrap());
    println!("Day 01 pt1 : {:}", result);
}

pub fn solve_part2() {
    let buffered = BufReader::new(File::open("./input/day01.txt").unwrap());
    let freqs : Vec<i32> = buffered.lines().map(|l| l.unwrap().parse::<i32>().unwrap()).collect();
