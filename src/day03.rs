...
impl FabricClaim {
    fn new(s: &String) -> FabricClaim {
        ...
        lazy_static! {
            static ref RE: Regex =
                Regex::new("#(\\d+)\\s@\\s(\\d+),(\\d+):\\s(\\d+)x(\\d+)").unwrap();
        }
...
pub fn solve_part1() {
    ...
    let fabric_claims: Vec<FabricClaim> = puzzle_input
        .lines()
        .map(|l| FabricClaim::new(&l.unwrap()))
        .collect();
    ...
    println!(
        "Day 03 pt 1: {}",
        fabric.values().filter(|&x| *x >= 2u16).count()
    );
}

pub fn solve_part2() {
    let puzzle_input = BufReader::new(File::open("./input/day03.txt").unwrap());
    let mut fabric_claims: Vec<FabricClaim> = puzzle_input
        .lines()
        .map(|l| FabricClaim::new(&l.unwrap()))
        .collect();
